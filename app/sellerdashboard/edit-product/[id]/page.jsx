...
const EditProduct = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Add new images - USE THE SAME FIELD NAME AS ADD PRODUCT
      files.forEach((file) => {
        if (file) {
          formData.append("image_files", file); // ← Changed from 'images' to 'image_files'
        }
      });

      // If you need to handle existing image deletions, you might need to send the remaining image IDs
      // For example, if your backend expects to know which existing images to keep:
      const remainingImageIds = existingImages.map(img => img.id);
      formData.append('existing_images', JSON.stringify(remainingImageIds));
