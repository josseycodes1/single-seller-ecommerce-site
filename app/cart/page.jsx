...
const Cart = () => {
  ...
  const [pendingUpdates, setPendingUpdates] = useState(new Map()); // Track pending updates
  ...
  // Process pending updates when component unmounts or user navigates away
  useEffect(() => {
    return () => {
  ...
  const processPendingUpdates = async () => {
    ...
    for (const [itemId, quantity] of updates) {
      try {
        await updateCartQuantity(itemId, quantity, false); // Don't show toast for batch updates
      } catch (error) {
  ...
  const debouncedUpdate = (itemId, quantity) => {
    ...
    // Add to pending updates (replaces any existing update for this item)
    setPendingUpdates(prev => {
      const newUpdates = new Map(prev);
      newUpdates.set(itemId, quantity);
      return newUpdates;
    });

    // Set a timeout to process updates after a delay
    setTimeout(processPendingUpdates, 1000); // Process after 1 second of inactivity
  };

  const handleUpdateQuantity = async (itemId, newQuantity, immediate = false) => {
    ...
    if (product && qty > product.stock) {
      ...
      setLocalQuantities(prev => ({
        ...prev,
        [itemId]: Math.min(cartItem.quantity, product.stock)
      }));
      return;
    }

    if (immediate) {
      // For explicit saves (like blur/enter), update immediately
      setIsUpdating(prev => ({ ...prev, [itemId]: true }));
      try {
      ...
      // For button clicks, use debounced updates
      debouncedUpdate(itemId, qty);
    }
  };
  ...
  const handleQuantityBlur = async (itemId) => {
    ...
    if (isNaN(qty) || qty < 1) {
      ...
      await handleUpdateQuantity(itemId, 1, true);
      return;
    }

    await handleUpdateQuantity(itemId, qty, true);
  };
  ...
  const handleIncrease = (cartItem) => {
    const currentQty = localQuantities[cartItem.id] || cartItem.quantity;
    const newQty = currentQty + 1;
    
    // Update optimistically without immediate API call
    handleUpdateQuantity(cartItem.id, newQty, false);
  };
  ...
  const confirmColorChange = async () => {
    ...
    try {
      ...
      const body = {
        cart_id: cart.id,
        quantity: localQuantities[cartItem.id] || cartItem.quantity, // Use local quantity
        color: selectedColor,
      };
  ...
  // Force process pending updates when proceeding to checkout
  const handleProceedToCheckout = async () => {
    if (pendingUpdates.size > 0) {
      await processPendingUpdates();
    }
    // The OrderSummary component will handle the actual navigation
  };
                    ...
                    {displayCart.items.map((cartItem) => {
                      ...
                      return (
                        <tr key={cartItem.id} className="border-b border-gray-200 hover:bg-gray-50">
                            ...
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                <button
                                  ...
                                  onClick={() => handleUpdateQuantity(cartItem.id, 0, true)}
                                >
                                  Remove
                                </button>
                              </div>
                          ...
                          <td className="py-6 px-4 text-gray-700 font-medium">
                            ${(parseFloat(productPrice) * currentQuantity).toFixed(2)}
                          </td>
                        </tr>
                      );
                    })}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex justify-between text-sm font-medium pt-2 border-t border-gray-200">
                              ...
                              <span className="text-gray-800">${(parseFloat(productPrice) * currentQuantity).toFixed(2)}</span>
                            </div>
                          </div>
                        </div>
                      </div>
